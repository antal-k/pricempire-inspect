...
class Postgres {
    ...
    async ensureSchema() {
        ...
        ALTER TABLE history ADD COLUMN IF NOT EXISTS current_steamid INTEGER;
        ...
        BEGIN
            ...
            IF NEW.a < OLD.a THEN
                ...
                INSERT INTO history VALUES (NEW.floatid, NEW.a, NEW.ms, NEW.updated, NULL, OLD.ms, OLD.stickers, 6) ON CONFLICT DO NOTHING;
                
                -- Prevent update to this row
                RETURN NULL;
            END IF;
 
            IF (is_steamid(OLD.ms) AND OLD.ms != NEW.ms) THEN -- OR OLD.price IS NOT NULL THEN
                -- We care about history for inventory changes or market listings that had price data 
                INSERT INTO history VALUES (NEW.floatid, OLD.a, OLD.ms, OLD.updated, OLD.price, NEW.ms, NEW.stickers, 1); 
            ELSEIF (is_steamid(OLD.ms) AND is_steamid(NEW.ms) IS FALSE) THEN
                -- Market Listing
                INSERT INTO history VALUES (NEW.floatid, OLD.a, OLD.ms, OLD.updated, OLD.price, NEW.ms, NEW.stickers, 2); 
            ELSEIF (is_steamid(OLD.ms) IS FALSE AND is_steamid(NEW.ms) IS FALSE) THEN
                -- Market Re-Listing
                INSERT INTO history VALUES (NEW.floatid, OLD.a, OLD.ms, OLD.updated, NEW.price, NEW.ms, NEW.stickers, 7); 
            ELSEIF (is_steamid(OLD.ms) IS FALSE AND is_steamid(NEW.ms)) THEN
                -- Market purchase
                INSERT INTO history VALUES (NEW.floatid, OLD.a, OLD.ms, OLD.updated, OLD.price, NEW.ms, NEW.stickers, 3); 
            ELSEIF (is_steamid(OLD.ms) AND OLD.stickers != NEW.stickers AND OLD.ms = NEW.ms) THEN
                ...
                INSERT INTO history VALUES (NEW.floatid, OLD.a, OLD.ms, OLD.updated, OLD.price, NEW.ms, NEW.stickers, 4); 
            ELSEIF (NEW.floatid = NEW.a) THEN
                -- New Item
                INSERT INTO history VALUES (NEW.floatid, NEW.a, NEW.ms, NEW.updated, NEW.price, NEW.ms, NEW.stickers, 5); 
            END IF;
    ...
    static buildQuery(itemCount) {
        ...
        console.log(`INSERT INTO items (ms, a, d, paintseed, paintwear, defindex, paintindex, stattrak, souvenir, props, stickers, updated, rarity, floatid, price)
