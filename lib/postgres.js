    ...
    async ensureSchema() {
        ...
        BEGIN
            ...
            IF NEW.a < OLD.a THEN
                ...
                INSERT INTO history VALUES (NEW.floatid, NEW.a, NEW.ms, NEW.updated, NULL, OLD.ms, OLD.stickers) ON CONFLICT DO NOTHING;
                
                -- Prevent update to this row
                RETURN NULL;
            END IF;

            -- is_steamid(OLD.ms) AND 
            IF (OLD.ms != NEW.ms) OR OLD.price IS NOT NULL THEN
                -- We care about history for inventory changes or market listings that had price data
                INSERT INTO history VALUES (NEW.floatid, OLD.a, OLD.ms, OLD.updated, OLD.price, NEW.ms, NEW.stickers);
            END IF;
