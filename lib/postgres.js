...
class Postgres {
    ...
    async ensureSchema() {
        ...
        ALTER TABLE history ADD COLUMN IF NOT EXISTS current_steamid BIGINT;
        ALTER TABLE history ADD COLUMN IF NOT EXISTS stickers jsonb;
        ALTER TABLE history ADD COLUMN IF NOT EXISTS type INTEGER;
        ALTER TABLE history ADD COLUMN IF NOT EXISTS d BIGINT;
        ALTER TABLE history ADD COLUMN IF NOT EXISTS stickers_new jsonb DEFAULT NULL;

        -- type: 1 = trade
        -- type: 2 = market listing
        -- type: 3 = market buy
        -- type: 4 = sticker apply
        -- type: 5 = unboxed/new item
        -- type: 6 = unknown
        -- type: 7 = market re-listing
        ...
        BEGIN
            ...
            IF NEW.a < OLD.a THEN
                ...
                INSERT INTO history VALUES (NEW.floatid, NEW.a, NEW.ms, NEW.updated, NULL, OLD.ms, OLD.stickers, 6, OLD.d) ON CONFLICT DO NOTHING;
                
                -- Prevent update to this row
                RETURN NULL;
            END IF;
 
            IF (is_steamid(OLD.ms) AND OLD.ms != NEW.ms AND is_steamid(NEW.ms)) THEN -- OR OLD.price IS NOT NULL THEN
                -- We care about history for inventory changes or market listings that had price data 
                INSERT INTO history VALUES (NEW.floatid, NEW.a, OLD.ms, NOW(), NULL, NEW.ms, NEW.stickers, 1, NEW.d); 
            ELSEIF (is_steamid(OLD.ms) AND is_steamid(NEW.ms) IS FALSE) THEN
                -- Market Listing
                INSERT INTO history VALUES (NEW.floatid, NEW.a, OLD.ms, NOW(), NEW.price, NEW.ms, NEW.stickers, 2, NEW.d); 
            ELSEIF (is_steamid(OLD.ms) IS FALSE AND is_steamid(NEW.ms) IS FALSE) THEN
                -- Market Re-Listing
                INSERT INTO history VALUES (NEW.floatid, NEW.a, OLD.ms, NOW(), NEW.price, NEW.ms, NEW.stickers, 7, NEW.d); 
            ELSEIF (is_steamid(OLD.ms) IS FALSE AND is_steamid(NEW.ms)) THEN
                -- Market purchase
                INSERT INTO history VALUES (NEW.floatid, NEW.a, OLD.ms, NOW(), OLD.price, NEW.ms, NEW.stickers, 3, NEW.d); 
            ELSEIF (is_steamid(OLD.ms) AND OLD.stickers != NEW.stickers AND OLD.ms = NEW.ms) THEN
                -- We care about sticker applications/removals too
                INSERT INTO history VALUES (NEW.floatid, NEW.a, OLD.ms, NOW(), NULL, NEW.ms, NEW.stickers, 4, NEW.d, OLD.stickers); 
            ELSEIF (NEW.floatid = NEW.a) THEN
                -- New Item
                INSERT INTO history VALUES (NEW.floatid, NEW.a, NEW.ms, NEW.updated, NEW.price, NEW.ms, NEW.stickers, 5, NEW.d); 
            END IF;
        ...
        await this.pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS i_unique_item ON 
                                items (defindex, paintindex, paintwear, paintseed, stattrak, souvenir)`);
        await this.pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS i_unique_fid ON items (floatid)`);
        await this.pool.query(`CREATE INDEX IF NOT EXISTS steam_ids ON history (steamid, current_steamid, floatid)`);
    }
    ...
    async handleBulkInsert(data) {
        ...
        for (let [item, price] of data) {
            ...
            const stickers = item.stickers.length > 0 ? item.stickers.map((s) => {
                const res = { s: s.slot, i: s.stickerId, r: s.rotation, sc: s.scale, t: s.tint_id };
                if (s.wear) {
                    res.w = s.wear;
                }
                return res;
        ...
        try {
            ...
            winston.debug(`executing bulk update: ${values.length}`);
    ...
    static buildQuery(itemCount) {
        ...
        return `INSERT INTO items (ms, a, d, paintseed, paintwear, defindex, paintindex, stattrak, souvenir, props, stickers, updated, rarity, floatid, price)
                VALUES ${values.join(', ')} ON CONFLICT(defindex, paintindex, paintwear, paintseed, stattrak, souvenir) DO UPDATE SET ms=excluded.ms, a=excluded.a, d=excluded.d, stickers=excluded.stickers, updated=now()`;
    }
    ...
    async _getItemData(links) {
        ...
        const result = await this.pool.query(`
            ...
            HighRank AS (
                SELECT
                    S.id AS item_id,
                    COUNT(*) + 1 AS high_rank
                FROM
                    items S
                    JOIN items J ON
                    J.paintwear > S.paintwear
            ...
            SELECT
                S.*,
                COALESCE(L.low_rank, 0) AS low_rank,
                COALESCE(H.high_rank, 0) AS high_rank
            FROM
                items S
            LEFT JOIN LowRank L ON S.id = L.item_id
            LEFT JOIN HighRank H ON S.id = H.item_id
            WHERE
                S.id = ANY($1::bigint[])`, [aValues]);
