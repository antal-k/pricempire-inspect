    ...
    async ensureSchema() {
        ...
        BEGIN
            ...
            IF (is_steamid(OLD.ms) AND OLD.ms != NEW.ms AND is_steamid(NEW.ms)) THEN -- OR OLD.price IS NOT NULL THEN
                ...
                INSERT INTO history VALUES (NEW.floatid, OLD.a, OLD.ms, NOW(), OLD.price, NEW.ms, NEW.stickers, 1); 
            ELSEIF (is_steamid(OLD.ms) AND is_steamid(NEW.ms) IS FALSE) THEN
                -- Market Listing
                INSERT INTO history VALUES (NEW.floatid, OLD.a, OLD.ms, NOW(), OLD.price, NEW.ms, NEW.stickers, 2); 
            ELSEIF (is_steamid(OLD.ms) IS FALSE AND is_steamid(NEW.ms) IS FALSE) THEN
                -- Market Re-Listing
                INSERT INTO history VALUES (NEW.floatid, OLD.a, OLD.ms, NOW(), NEW.price, NEW.ms, NEW.stickers, 7); 
            ELSEIF (is_steamid(OLD.ms) IS FALSE AND is_steamid(NEW.ms)) THEN
                -- Market purchase
                INSERT INTO history VALUES (NEW.floatid, OLD.a, OLD.ms, NOW(), OLD.price, NEW.ms, NEW.stickers, 3); 
            ELSEIF (is_steamid(OLD.ms) AND OLD.stickers != NEW.stickers AND OLD.ms = NEW.ms) THEN
                ...
                INSERT INTO history VALUES (NEW.floatid, OLD.a, OLD.ms, NOW(), OLD.price, NEW.ms, NEW.stickers, 4); 
