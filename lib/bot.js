...
class Bot extends EventEmitter {
    ...
    newSession() {
        this.session = Math.round(Math.random() * 10000000);
    }
    ...
    init() {
        ...
        winston.debug(`${this.username} Initialize`);
        const proxy = CONFIG.proxy_url.replace('[session]', this.session || this.username /* this.session*/);

        if (proxy.startsWith('http://')) {
            this.settings.steam_user = Object.assign({}, this.settings.steam_user, { httpProxy: proxy });
        } else if (proxy.startsWith('socks5://')) {
            this.settings.steam_user = Object.assign({}, this.settings.steam_user, { socksProxy: proxy });
        }
    ...
    logIn(username, password, auth, session) {
        ...
        winston.debug(`Logging in ${this.username}`);
    ...
    bindEventHandlers() {
        ...
        this.steamClient.on('disconnected', (eresult, msg) => {
            winston.debug(`${this.username} Logged off, reconnecting! (${eresult}, ${msg})`);
        });

        this.steamClient.on('loggedOn', (details, parental) => {
            winston.debug(`${this.username} Log on OK`);
            ...
            if (this.relogin) {
                ...
                winston.debug(`${this.username} Initiating GC Connection, Relogin`);
            ...
            this.steamClient.once('appOwnershipCached', () => {
                if (!this.steamClient.ownsApp(730)) {
                    winston.debug(`${this.username} doesn't own CS:GO, retrieving free license`);
                    ...
                    this.steamClient.requestFreeLicense([730], (err, grantedPackages, grantedAppIDs) => {
                        ...
                        if (err) {
                            ...
                            winston.debug(`${this.username} Initiating GC Connection`);
                            this.steamClient.gamesPlayed([730], true);
                        }
                    });
                } else {
        ...
        this.csgoClient.on('connectedToGC', () => {
            winston.debug(`${this.username} CSGO Client Ready!`);
            this.ready = true;
        });

        this.csgoClient.on('disconnectedFromGC', (reason) => {
            winston.debug(`${this.username} CSGO unready (${reason}), trying to reconnect!`);
            this.ready = false;
    ...
    sendFloatRequest(link) {
        return new Promise((resolve, reject) => {
            ...
            this.csgoClient.inspectItem(params.s !== '0' ? params.s : params.m, params.a, params.d, (iteminfo) => {
